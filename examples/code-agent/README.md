...
and explicit whitespace-tolerant strategies for indentation drift. Use `file.edit` with `edits[]`
for atomic multi-point changes in one file; if any edit fails, the file is left unchanged. Each edit
still emits bounded diff output for audit,
