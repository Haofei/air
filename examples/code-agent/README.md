...
string but should not guess line numbers, `file.edit` for exact-string changes that require a prior
read by default and explicit whitespace-tolerant strategies for indentation drift. Use `file.edit`
with `edits[]` for atomic multi-point changes in one file; if any edit fails, the file is left
unchanged. Each edit still emits bounded diff output for audit,
