...
echo "[air-conformance] tool batch approval isolation matrix"
cargo run -q -p air-cli -- validate-plan tests/plans/tool-batch-approval-smoke.air-plan.yaml \
  --store tests/plans/tool-batch-approval-smoke.air-store.yaml
set +e
cargo run -q -p air-cli -- run-plan tests/plans/tool-batch-approval-smoke.air-plan.yaml \
  --store tests/plans/tool-batch-approval-smoke.air-store.yaml \
  --input tests/plans/tool-batch-approval-smoke.input.json \
  --tool-config tests/plans/tool-batch-approval-smoke.tools.json \
  --trace-out "$OUT/tool_batch_approval.native.trace.jsonl" \
  > "$OUT/tool_batch_approval.native.output.json" \
  2> "$OUT/tool_batch_approval.native.stderr.txt"
native_batch_approval_status=$?
set -e
if [[ "$native_batch_approval_status" -eq 0 ]]; then
  echo "expected native runtime to reject mixed approval-required tool batch" >&2
  exit 1
fi
grep -q "approval-required capability file.write" "$OUT/tool_batch_approval.native.stderr.txt"
cargo run -q -p air-cli -- lower-plan tests/plans/tool-batch-approval-smoke.air-plan.yaml \
  --store tests/plans/tool-batch-approval-smoke.air-store.yaml \
  --backend openai-js-strict \
  --output "$OUT/tool_batch_approval.openai.mjs"
set +e
AIR_TRACE=1 node "$OUT/tool_batch_approval.openai.mjs" \
  --input tests/plans/tool-batch-approval-smoke.input.json \
  --model-config "$MODEL_CONFIG" \
  --tool-config tests/plans/tool-batch-approval-smoke.tools.json \
  > "$OUT/tool_batch_approval.openai.output.json" \
  2> "$OUT/tool_batch_approval.openai.stderr.txt"
openai_batch_approval_status=$?
set -e
if [[ "$openai_batch_approval_status" -eq 0 ]]; then
  echo "expected OpenAI JS strict runtime to reject mixed approval-required tool batch" >&2
  exit 1
fi
grep -q "approval-required capability file.write" "$OUT/tool_batch_approval.openai.stderr.txt"
cargo run -q -p air-cli -- lower-plan tests/plans/tool-batch-approval-smoke.air-plan.yaml \
  --store tests/plans/tool-batch-approval-smoke.air-store.yaml \
  --backend langgraph \
  --output "$OUT/tool_batch_approval.langgraph.py"
PYTHONWARNINGS=ignore "$LANGGRAPH_PYTHON" - <<'PY'
import importlib.util
import json

spec = importlib.util.spec_from_file_location("tool_batch_approval_conformance", "target/generated/conformance/tool_batch_approval.langgraph.py")
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
module.AIR_APPROVAL_PROVIDER = lambda **kwargs: {
    "approved": True,
    "approver": "conformance",
    "reason": "approval isolation conformance test",
}
inputs = json.load(open("tests/plans/tool-batch-approval-smoke.input.json", encoding="utf-8"))
try:
    module.graph.invoke(inputs)
except RuntimeError as error:
    assert "approval-required capability file.write" in str(error), error
else:
    raise AssertionError("expected LangGraph runtime to reject mixed approval-required tool batch")
PY
