...
#[derive(Clone)]
enum ToolProviderChoice {
    Echo(EchoTools),
    Config(ConfigTools),
}

impl ToolProviderChoice {
    fn from_config(tool_config: Option<PathBuf>, example_tools: bool) -> Result<Self> {
...
fn validate_tool_config(config: &ToolConfigFile, path: &std::path::Path) -> Result<()> {
...
impl ToolProvider for ConfigTools {
    fn call_tool(&mut self, name: &str, input: &Value) -> Result<Value, RuntimeError> {
        ...
        match tool {
            ...
            ToolConfig::LocalReflection { capability: _ } => Ok(serde_json::json!({
                "reflection": input
                    .get("reflection")
                    .cloned()
...
fn search_docs(input: &Value, documents: &[LocalDoc], max_results: usize) -> Value {
    let query = input
        .get("query")
        .and_then(Value::as_str)
        .unwrap_or_default()
