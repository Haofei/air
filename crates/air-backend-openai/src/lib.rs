...
fn opencode_bash_description() -> String {
    let directory = std::env::current_dir()
        ...
        .unwrap_or_else(|_| ".".to_string());
    format!(
        "Run a shell command for terminal operations such as git status, package manager commands, builds, tests, and linters.\n\nDefault working directory: {directory}. Use the `workdir` argument instead of `cd`.\n\nRequired arguments:\n- `command`: command string to run.\n- `description`: concise 5-10 word description.\n\nOptional arguments:\n- `workdir`: directory for the command.\n- `timeout`: timeout in milliseconds.\n\nUse dedicated tools for file search, file reads, and file edits. Do not use bash for routine source inspection or string replacement. Output may be truncated after 2000 lines or 51200 bytes."
    )
}
...
fn validate_native_tool_arguments(tool: &str, input: &Value) -> Result<(), RuntimeError> {
    let Some(object) = input.as_object() else {
        return Err(RuntimeError::Provider(format!(
            "malformed native tool call for {tool}: arguments must be a JSON object"
        )));
    };
    let required = native_tool_required_fields(tool);
    ...
    let provided = compact_json(input);
    let missing_list = missing.join(", ");
    let required_list = required.join(", ");
    if object.is_empty() {
        return Err(RuntimeError::Provider(format!(
            "malformed native tool call for {tool}: empty arguments {{}} missing required fields [{required_list}]. Tool calls must include concrete arguments and cannot be placeholders."
        )));
    }
    Err(RuntimeError::Provider(format!(
        "malformed native tool call for {tool}: arguments {provided} missing required fields [{missing_list}]. Required fields: [{required_list}]."
    )))
}

fn native_tool_required_fields(tool: &str) -> &'static [&'static str] {
    match tool {
        "question" => &["questions"],
        "read_range" => &["file", "offset", "limit"],
        "read_contains" => &["file", "contains", "context_lines"],
...
mod tests {
    ...
    #[test]
    fn opencode_bash_description_stays_tool_specific() {
        let description = opencode_bash_description();

        assert!(description.len() < 1200);
        assert!(!description.contains("TodoWrite"));
        assert!(!description.contains("Creating pull requests"));
        assert!(description.contains("Required arguments"));
    }
    ...
    fn native_tool_calls_reject_empty_required_arguments() {
        ...
        assert!(message.contains("empty arguments {}"), "{message}");
        assert!(
            message.contains("filePath")
                && message.contains("oldString")
                && message.contains("newString"),
            "{message}"
        );
    }
