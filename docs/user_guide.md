...
| `file_read` | `file.read` | `{ "path": "relative/file.txt", "start_line": 10, "end_line": 80, "line_numbers": true }` | `{ path, content, numbered_content, bytes, start_line, end_line, total_lines, truncated, artifacts[] }` | Read-only and constrained to configured `base_dir`; line range and numbered output are optional. Binary and non-UTF-8 files are rejected instead of lossy-decoded into agent context. |
...
| `file_edit` | `file.edit` | `{ "path": "relative/file.txt", "old_string": "...", "new_string": "...", "match_strategy": "exact" }` | `{ path, bytes, replacements, match_strategy, diff, diff_truncated, artifacts[] }` | Edit constrained to configured `base_dir`; requires a fresh prior `file.read` by default. The default `exact` strategy uses literal matching; explicit `line_trimmed` or `indentation_flexible` strategies support conservative whitespace-tolerant block replacement. Emits bounded unified-diff output for audit. No-op edits and ambiguous multiple matches are rejected unless `replace_all` is explicitly allowed. |
...
| `repo_search` | `repo.search` | `{ "query": "...", "mode": "fixed", "path": "optional/dir" }` | `{ repo, query, mode, matches[], truncated, artifacts[] }` | Read-only search through `rg`; defaults to fixed-string mode and supports explicit `mode: "regex"` for grep-style code discovery. Returns path/line/column/text matches. |
