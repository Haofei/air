...
| `file_read` | `file.read` | `{ "path": "relative/file.txt", "start_line": 10, "end_line": 80, "line_numbers": true }` or `{ "path": "...", "contains": "symbol", "context_lines": 8 }` | `{ path, content, numbered_content, bytes, start_line, end_line, match_line, contains, total_lines, truncated, artifacts[] }` | Read-only and constrained to configured `base_dir`; line ranges, numbered output, and fixed-string `contains` locators are optional. `contains` returns the first matching line plus bounded surrounding context and fails if the string is absent, preventing accidental unrelated reads. Binary and non-UTF-8 files are rejected instead of lossy-decoded into agent context. |
