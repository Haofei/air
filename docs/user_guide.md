...
| `file_read` | `file.read` | `{ "path": "relative/file.txt", "start_line": 10, "end_line": 80 }` | `{ path, content, bytes, start_line, end_line, total_lines, truncated, artifacts[] }` | Read-only and constrained to configured `base_dir`; line range is optional. Binary and non-UTF-8 files are rejected instead of lossy-decoded into agent context. |
